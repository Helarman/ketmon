...
export default async function getLandlordRating({ id }: { id: number }) {
    try {
        const res = await axios.get(`https://dazzling-symphony-5fc97c5a00.strapiapp.com/api/reviews?filters[landlord][id][$eqi]=${id}&populate=*`);
