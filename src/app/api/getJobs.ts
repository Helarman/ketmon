...
export default async function getJobs({ searchParams }: { searchParams: any }) {
    ...
    try {
        const res = await axios.get(`https://excellent-chickens-fb25f11199.strapiapp.com/api/jobs?pagination[page]=${currentPage}&pagination[pageSize]=5&${searchQuery}populate=*`);
        const jobs = res.data;
